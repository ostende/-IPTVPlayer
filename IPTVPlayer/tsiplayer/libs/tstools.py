...
class TSCBaseHostClass:
    ...
    def start(self,cItem):
        ...
        if mode=='00':
            ...
            self.showelms(cItem)		
        return True
    def add_menu(self, cItem, pat1, pat2, data, mode_,s_mode=[], del_=[], TAB=[], search=False, Titre='',ord=[0,1],Desc=[],Next=[0,0],u_titre=False,ind_0=0,local=[],resolve='0',EPG=False,corr_=True,pref_='',post_data='',pat3='',ord3=[0,1],LINK=''):
        ...
        if TAB!=[]:
                        ...
                        if mode=='serv': 
                            ...
                            if not any(word in titre for word in del_):
                                ...
                                if titre!='':
                                    ...
                                    if mode=='video':
                                        found = True
                                        self.addVideo({'category':'host2','good_for_fav':True, 'title': titre,'sub_mode':sub_mode,'url':url, 'desc':desc,'import':cItem['import'],'icon':image,'hst':'tshost','EPG':EPG})	
                                    else:	
            ...
            if (mode=='video') and (not found):
                self.addVideo({'category':'host2','good_for_fav':True, 'title': cItem['title'],'url':cItem['url'], 'desc':cItem['desc'],'import':cItem['import'],'icon':cItem['icon'],'hst':'tshost','EPG':EPG})						
        if search:
