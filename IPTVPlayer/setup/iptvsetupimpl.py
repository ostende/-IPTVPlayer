# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/IPTVPlayer/setup/iptvsetupimpl.py
...
class IPTVSetupImpl:
    ...
    def __init__(self, finished, chooseQuestion, showMessage, setInfo):
        ...
        self.rtmpdumpVersion = 20151215
        self.rtmpdumppaths = ['/usr/bin/rtmpdump', 'rtmpdump']
        self.f4mdumpVersion = 0.8
        self.f4mdumppaths = ['/usr/bin/f4mdump', GetBinDir('f4mdump', '')]
        self._f4mdumpInstallChoiseList = [(_('Install into the "%s".') % ('/usr/bin/f4mdump (%s)' % _('recommended')), '/usr/bin/f4mdump'), (_('Install into the "%s".') % 'IPTVPlayer/bin/f4mdump', GetBinDir('f4mdump', '')), (_('Do not install (not recommended)'), '')]
        self._f4mdumpInstallChoiseList2 = [(_('Install into the "%s".') % ('/usr/bin/f4mdump static libstdc++ (%s)' % _('recommended')), '/usr/bin/f4mdump'), (_('Install into the "%s".') % 'IPTVPlayer/bin/f4mdump _static_libstdc++', GetBinDir('f4mdump', '')), (_('Do not install (not recommended)'), '')]
        self.uchardetVersion = [0, 0, 6]
        self.uchardetpaths = ['/usr/bin/uchardet', GetBinDir('uchardet', '')]
        self._uchardetInstallChoiseList = [(_('Install into the "%s".') % ('/usr/bin/uchardet (%s)' % _('recommended')), '/usr/bin/uchardet'), (_('Install into the "%s".') % 'IPTVPlayer/bin/uchardet', GetBinDir('uchardet', '')), (_('Do not install (not recommended)'), '')]
        self._uchardetInstallChoiseList2 = [(_('Install into the "%s".') % ('/usr/bin/uchardet static libstdc++ (%s)' % _('recommended')), '/usr/bin/uchardet'), (_('Install into the "%s".') % 'IPTVPlayer/bin/uchardet _static_libstdc++', GetBinDir('uchardet', '')), (_('Do not install (not recommended)'), '')]
        self.gstplayerVersion = {'0.10': 20,
         '1.0': 10021}
        self.gstplayerpaths = ['/usr/bin/gstplayer', GetBinDir('gstplayer', '')]
        self._gstplayerInstallChoiseList = [(_('Install into the "%s".') % ('/usr/bin/gstplayer (%s)' % _('recommended')), '/usr/bin/gstplayer'), (_('Install into the "%s".') % 'IPTVPlayer/bin/gstplayer', GetBinDir('gstplayer', '')), (_('Do not install (not recommended)'), '')]
        self.exteplayer3Version = {'sh4': 50,
         'mipsel': 50,
         'armv7': 50,
         'armv5t': 50}
        self.exteplayer3paths = ['/usr/bin/exteplayer3', GetBinDir('exteplayer3', '')]
        self._exteplayer3InstallChoiseList = [(_('Install into the "%s".') % ('/usr/bin/exteplayer3 (%s)' % _('recommended')), '/usr/bin/exteplayer3'), (_('Install into the "%s".') % 'IPTVPlayer/bin/exteplayer3', GetBinDir('exteplayer3', '')), (_('Do not install (not recommended)'), '')]
    ...
    def platformDetect(self):
        ...
        for platform in self.supportedPlatforms:
            ...
            try:
                os_chmod(platformtesterPath, 511)
            except Exception:
                printExc()
    ...
    def detectFPU(self):
        ...
        if config.plugins.iptvplayer.plarform.value != 'mipsel' or IsFPUAvailable() or config.plugins.iptvplayer.plarformfpuabi.value != '':
            ...
            for resourceServer in self.resourceServers:
                outCmdTab.append('wget -q "%sbin/mipsel/fputest" -O "%s/fputest" ; chmod 777 "%s/fputest" ; "%s/fputest" ; rm "%s/fputest" 2>/dev/null' % (resourceServer,
                 self.tmpDir,
                 self.tmpDir,
                 self.tmpDir,
                 self.tmpDir))
    ...
    def uchardetStep(self, ret = None):
        ...
        def _detectValidator(code, data):
            ...
            try:
                ...
                if UCHARDET_VERSION_MAJOR > self.uchardetVersion[0] or UCHARDET_VERSION_MAJOR == self.uchardetVersion[0] and UCHARDET_VERSION_MINOR > self.uchardetVersion[1] or UCHARDET_VERSION_MAJOR == self.uchardetVersion[0] and UCHARDET_VERSION_MINOR == self.uchardetVersion[1] and UCHARDET_VERSION_REVISION >= self.uchardetVersion[2]:
                    return (True, False)
            except Exception:
                printExc()
    ...
    def getPackageName(self, name):
        ...
        try:
            ...
            if self.platform == 'mipsel' and remoteBinaryName in ('python2.6_{0}_old', 'python2.6_{0}_old_softfpu', 'python2.7_{0}', 'python2.7_{0}_old', 'python2.7_{0}_old_softfpu', 'python2.7_{0}_softfpu'):
        ...
        if packageAvailable:
            return remoteBinaryName.format(name)
        return ''
