...
class ForjaTN(CBaseHostClass):
    ...
    def __init__(self):
        CBaseHostClass.__init__(self, {'history': 'forja.tn',
         'cookie': 'forja.tn.cookie'})
    ...
    def listItems(self, cItem, nextCategory):
        ...
        try:
            ...
            for item in data[type]:
                ...
                title = ph.clean_html(item.get('Title', ''))
                desc = []
                for t in ['Year', 'imdbRating', 'Genre']:
                    t = ph.clean_html(item.get(t, ''))
                    if t != '':
                        desc.append(t)

                desc = ' | '.join(desc) + '[/br]' + ph.clean_html(item.get('Plot', ''))
                id = item.get('_id', '')
    ...
    def exploreItem(self, cItem, nextCategory):
        ...
        if not sts:
            return
        errMsg = ph.clean_html(ph.find(data, ('<div', '>', 'medias_container'), '</div>', flags=0)[1])
        printDBG('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+ ' + errMsg)
        if type == 'series':
            seasonsTab = []
            ...
            for item in data:
                ...
                title = '%s - %s' % (cItem['title'], ph.clean_html(self.cm.ph.getSearchGroups(item, 'episode\\-name=[\'"]([^\'^"]+?)[\'"]')[0]))
                if season not in seasonsTab:
                    self.cacheEpisodes[season] = []
                    seasonsTab.append(season)
                self.cacheEpisodes[season].append({'url': url,
                 'title': title,
                 'icon': icon})
    ...
    def getLinksForVideo(self, cItem):
        ...
        if cItem.get('f_type', '') == 'episode':
                    ...
                    for eItem in data:
                        ...
                        for item in eItem['textTracks']:
                            if item.get('kind', '') != 'captions':
                                continue
                            subTracksTab.append({'title': item['label'],
                             'url': self.getFullUrl(item['src']),
                             'lang': item['language'],
                             'format': 'vtt'})
            ...
            for tmp in tmpTab:
                ...
                if 'x-mpegurl' in vidType:
                    ...
                    retTab.append({'name': 'mp4 %s' % vidLabel,
                     'url': vidUrl})
            ...
            for tmp in data:
                ...
                for item in tmp:
                    ...
                    subTracksTab.append({'title': title,
                     'url': url,
                     'lang': lang,
                     'format': 'vtt'})
        ...
        for idx in range(len(retTab)):
            retTab[idx]['url'] = strwithmeta(retTab[idx]['url'], {'User-Agent': self.USER_AGENT,
             'Referer': cItem['url'],
             'Cookie': cookieHeader,
    ...
    def getArticleContent(self, cItem, data = None):
        ...
        desc = ph.clean_html(self.cm.ph.getDataBeetwenMarkers(data, '<p', '</p>')[1])
        title = ph.clean_html(self.cm.ph.getDataBeetwenMarkers(data, '<h4', '</h4>')[1])
        ...
        for item in data:
            ...
            if len(item) != 2:
                continue
            marker = ph.clean_html(item[0]).lower()
