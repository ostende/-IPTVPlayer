...
class TrailersApple(CBaseHostClass):
    ...
    def __init__(self):
        CBaseHostClass.__init__(self, {'history': 'TrailersApple',
         'cookie': 'TrailersApple.cookie'})
    ...
    def listMain(self, cItem):
        ...
        MAIN_CAT_TAB = [{'category': 'list_items',
          'title': 'Just Added',
          'url': self.getFullUrl('/trailers/home/feeds/just_added.json')},
         {'category': 'list_items',
          'title': 'Exclusive',
          'url': self.getFullUrl('/trailers/home/feeds/exclusive.json')},
         {'category': 'list_items',
          'title': 'Just HD',
          'url': self.getFullUrl('/trailers/home/feeds/just_hd.json')},
         {'category': 'list_items',
          'title': 'Most Popular',
          'url': self.getFullUrl('/trailers/home/feeds/most_pop.json')},
         {'category': 'list_items',
          'title': 'Movie Studios',
          'url': self.getFullUrl('/trailers/home/feeds/studios.json')},
         {'category': 'search',
          'title': _('Search'),
          'search_item': True},
            ...
            for item in data['clips']:
                ...
                for version, versionData in item.get('versions', {}).iteritems():
                    for size, sizeData in versionData.get('sizes', {}).iteritems():
                        ...
                        if not url:
                            continue
                        urls.append({'name': '%s-%s' % (version, size),
                         'url': self.getFullUrl(re.sub('_(\\d+p\\.mov)', '_h\\1', url)),
