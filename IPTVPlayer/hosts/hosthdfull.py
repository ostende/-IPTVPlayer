# Embedded file name: /IPTVPlayer/hosts/hosthdfull.py
...
class SuggestionsProvider():
    ...
    def __init__(self):
        ...
        self.defaultParams = {'header': self.HTTP_HEADER,
         ...
         'cookie_items': {'language': self.lang}}
    ...
    def getSuggestions(self, text, locale):
        ...
        sts, data = self.cm.getPage(url, post_data={'q': text,
         'limit': '10',
...
class HDFull(CBaseHostClass, CaptchaHelper):
    ...
    def listMain(self, cItem):
        ...
        for menuData in data:
            ...
            subItems = []
            if category:
                subItems = [MergeDicts(cItem, {'url': menuUrl,
                  'title': _('All'),
                  'category': category})]
            for item in menuData:
                ...
                if category:
                    ...
                    tmp = url.split('#', 1)
                    if len(tmp) == 2:
                        params.update({'category': 'list_episodes_langs',
                         'url': tmp[0],
                         'f_action': tmp[1]})
                    else:
            ...
            if len(subItems):
                self.addDir(MergeDicts(cItem, {'url': menuUrl,
                 'title': menuTitle,
                 'category': 'sub_items',
                 'sub_items': subItems}))
    ...
    def listSortMoviesSeries(self, cItem, nextCategory1, nextCategory2):
        ...
        for item in data:
            ...
            if hasABCMenu and category == 'abc':
                ...
                continue
            title = self.cleanHtmlStr(item)
            self.addDir(MergeDicts(cItem, {'url': url,
             'title': title,
             'category': category,
             'fix_next_page': fixNextPage}))
    ...
    def listItems(self, cItem, nextCategory):
        ...
        if nextPage and len(self.currList):
            self.addDir(MergeDicts(cItem, {'url': nextPage,
             'title': _('Next page'),
             'page': page + 1}))
    ...
    def _getLinks(self, cUrl, data):
        ...
        for item in tmp:
            ...
            if 'providers' in item:
                tabJs['providers'] = {'url': self.getFullUrl(item),
                 'hash': version + '.2'}
            elif 'view' in item:
                tabJs['view'] = {'url': self.getFullUrl(item),
                 'hash': version + '.2'}
    ...
    def listEpisodes(self, cItem):
        ...
        cItem = MergeDicts(cItem, {'category': 'list_episodes2',
         'url': cUrl,
         'f_action': 'season',
         'f_show': sid,
         'f_season': cUrl.rsplit('-', 1)[-1]})
    ...
    def listEpisodes2(self, cItem):
        ...
        post_data = {'action': cItem['f_action'],
         'start': page * ITEMS,
         'limit': ITEMS}
        if 'f_show' in cItem:
        ...
        params['header'] = MergeDicts(params['header'], {'Referer': cItem['url'],
        ...
        try:
            ...
            for item in data:
                ...
                title = '%s - s%se%s %s' % (jstr(item['show']['title'], lang),
                 sNum.zfill(2),
                 eNum.zfill(2),
                ...
                self.addVideo({'good_for_fav': True,
                 'title': title,
                 'url': url,
                 'icon': icon,
                 'desc': desc})
