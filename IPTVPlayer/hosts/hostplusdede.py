# Embedded file name: /IPTVPlayer/hosts/hostplusdede.py
...
class PlusDEDE(CBaseHostClass):
    ...
    def __init__(self):
        CBaseHostClass.__init__(self, {'history': 'plusdede.com',
         'cookie': 'plusdede.com.cookie'})
        self.DEFAULT_ICON_URL = 'https://www.plusdede.net/wp-content/uploads/2019/02/megadedeapk.jpg'
        ...
        self.MAIN_CAT_TAB = [{'category': 'list_filters',
          'title': 'Series',
          'url': self.getFullUrl('/series')},
         {'category': 'list_filters',
          'title': 'Pelis',
          'url': self.getFullUrl('/pelis')},
         {'category': 'list_lists',
          'title': 'Listas',
          'url': self.getFullUrl('/listas')},
         {'category': 'search',
          'title': _('Search'),
          'search_item': True},
        ...
        if not sts:
                ...
                for item in data:
                    ...
                    if value == '':
                        if allTitle == None:
                            allTitle = title
                        continue
                    self.cacheFilters[key].append({'title': title.title(),
                     key: value})
            ...
            for idx in range(len(data)):
                ...
                if key in ('f_year',):
                    try:
                        ...
                        for val in range(start, end - 1, -1):
                            self.cacheFilters[key].append({'title': str(val),
                             key: '%s;%s' % (val, val)})
                ...
                if [] != self.cacheFilters.get(key, []):
                    ...
                    if len(title):
                        self.cacheFilters[key].insert(0, {'title': title,
                         'type': 'marker'})
        ...
        if 'season-links' in data:
            ...
            for season in sData:
                ...
                for item in eData:
                    ...
                    episodesTab.append({'title': title,
                     'url': url,
                     'desc': '[/br]'.join(desc)})
    ...
    def getLinksForVideo(self, cItem):
        ...
        for idx in range(len(data)):
            ...
            for item in dataItem:
                ...
                if idx == 0:
                    retTab.append({'name': '%s' % ' | '.join(titleTab),
                     'url': self.getFullUrl(url),
                     'need_resolve': 1})
                else:
                    dwnTab.append({'name': '%s' % ' | '.join(titleTab),
                     'url': self.getFullUrl(url),
                     'need_resolve': 1})
    ...
    def getArticleContent(self, cItem, data = None):
        ...
        otherInfo['duration'] = self.cleanHtmlStr(self.cm.ph.getDataBeetwenNodes(data, ('<strong', '</strong>', 'Duraci\xc3\xb3n'), ('</div', '>'), False)[1])
        tmp = self.cm.ph.getDataBeetwenNodes(data, ('<strong', '</strong>', 'G\xc3\xa9nero'), ('</ul', '>'), False)[1]
        tmp = self.cm.ph.getAllItemsBeetwenMarkers(tmp, '<li', '</li>')
        tmpTab = []
        for t in tmp:
            t = self.cleanHtmlStr(t)
    ...
    def tryTologin(self):
        ...
        if PlusDEDE.login == None and PlusDEDE.password == None:
            if self.readLoginMarker() == self.calcLoginMarker(config.plugins.iptvplayer.plusdede_login.value, config.plugins.iptvplayer.plusdede_password.value):
                ...
                PlusDEDE.login = ''
                rm(self.COOKIE_FILE)
        if True != self.loggedIn or PlusDEDE.login != config.plugins.iptvplayer.plusdede_login.value or PlusDEDE.password != config.plugins.iptvplayer.plusdede_password.value:
            if True != self.loggedIn and PlusDEDE.login == config.plugins.iptvplayer.plusdede_login.value and PlusDEDE.password == config.plugins.iptvplayer.plusdede_password.value:
            ...
            post_data.update({'email': PlusDEDE.login,
            ...
            if self.cm.isValidUrl(imgUrl):
                ...
                params = dict(self.defaultParams)
                params.update({'maintype': 'image',
                 'subtypes': ['jpeg', 'png'],
                 'check_first_bytes': ['\xff\xd8', '\xff\xd9', '\x89PNG'],
                 'header': header})
