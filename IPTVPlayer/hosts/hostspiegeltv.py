...
import urlparse, time, re, hashlib, urllib, random
...
class SpiegelTv(CBaseHostClass):
    ...
    def _initiSession(self, cItem):
        try:
            deviceId = '%d:%d:%d%d' % (random.randint(1, 4), int(time.time()), random.randint(10000.0, 99999), random.randint(1, 9))
            urlParams = dict(self.defaultParams)
            urlParams['header'] = dict(self.AJAX_HEADER)
            urlParams['header'].update({'Referer': cItem['url'], 'X-Request-Enable-Auth-Fallback': '1'})
            post_data = {'nxp_devh': deviceId, 'nxp_userh': '', 
               'precid': '0', 
               'playlicense': '0', 
               'screenx': '1920', 
               'screeny': '1080', 
    ...
    def listLiveVideos(self, cItem):
        ...
        try:
            ...
            post_data = {'additionalfields': 'language,channel,actors,studio,licenseby,slug,fileversion,subtitle,teaser,description,releasedate', 'addInteractionOptions': '1', 
               'addStatusDetails': '1', 
               'addStreamDetails': '1', 
               'addFeatures': '1', 
               'addCaptions': '1', 
    ...
    def getLinksForVideo(self, cItem):
        ...
        try:
            ...
            urlParams['header'] = dict(self.AJAX_HEADER)
            urlParams['header'].update({'Referer': cItem['url'], 'X-Request-Enable-Auth-Fallback': '1', 'X-Request-CID': cid, 'X-Request-Token': requestToken})
            post_data = {'additionalfields': 'language,channel,actors,studio,licenseby,slug,subtitle,teaser,description', 'addInteractionOptions': '1', 
               'addStatusDetails': '1', 
               'addStreamDetails': '1', 
               'addCaptions': '1', 
               'addScenes': '1', 
            ...
            if cdn == 'azure':
                ...
                def p0(p):
                    if streamData['applyAzureStructure'] == '1':
                        return '_%s' % p
                    return ''
